...
Current authorization: R8 is awaiting user review after Python consolidation, augmentation verification, and native contract cleanup.
...
| 2026-08-07 | R8 | Native-backed Python validation | The freshly built extension initializes the shared LibTorch inter-op pool before a spawned trainer rank configures PyTorch, and the Go objective derived float32 target labels from bfloat16 model logits. | Avoid resetting inter-op threads when the requested value is already active, and keep Go outcome/MCTS target interpolation in the replay target dtype. The complete extension-backed Python suite now passes 354 tests, including Go checkpoint publication. |
