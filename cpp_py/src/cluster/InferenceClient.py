...
from src.settings import TORCH_DTYPE, USE_GPU, ChessBoard, ChessGame, log_histogram, log_scalar
...
class InferenceClient:
    ...
    def __init__(self, device_id: int, network_args: NetworkParams, save_path: str) -> None:
        ...
        channels, rows, cols = ChessGame.representation_shape()
    ...
    def inference_batch(self, boards: list[ChessBoard]) -> list[tuple[list[MoveScore], float]]:
        if not boards:
            return []

        encoded_boards = [ChessGame.get_canonical_board(board) for board in boards]
        board_hashes = self.hasher.zobrist_hash_boards(np.array(encoded_boards))

        boards_to_infer: list[np.ndarray] = []
        inference_hashes_and_boards: list[tuple[int, ChessBoard]] = []
    ...
    def _model_inference(self, boards: list[np.ndarray]) -> list[tuple[np.ndarray, float]]:
        if self.model is None:
            ...
            return [(np.full((ChessGame.action_size(),), 1 / ChessGame.action_size()), 0.0) for _ in boards]
