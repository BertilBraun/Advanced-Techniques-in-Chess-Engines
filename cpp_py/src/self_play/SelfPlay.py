...
from src.settings import ChessBoard, ChessGame, ChessMove, log_text
...
class SelfPlay:
            ...
            for _ in range(count):
                ...
                while np.sum(spg_action_probabilities) > 0:
                    ...
                    if self.self_play_games[new_spg] == 0 and move not in spg.played_moves[-16:]:
                        ...
                        spg_action_probabilities[ChessGame.encode_move(move)] = 0
    ...
    def _sample_self_play_game(
        ...
        if len(current.played_moves) >= self.args.num_moves_after_which_to_play_greedy:
            move = ChessGame.decode_move(np.argmax(action_probabilities).item())
        else:
            move = self._sample_move(action_probabilities, self.args.temperature)

        new_spg = current.expand(move)
        # TODO encoded_move = ChessGame.encode_move(move)
    ...
    def _sample_move(self, action_probabilities: np.ndarray, temperature: float = 1.0) -> ChessMove:
        ...
        action = np.random.choice(ChessGame.action_size(), p=temperature_action_probabilities)

        return ChessGame.decode_move(action)
    ...
    def _add_training_data(self, spg: SelfPlayGame, game_outcome: float, resignation: bool) -> None:
        ...
        for mem in spg.memory[::-1]:
            encoded_board = ChessGame.get_canonical_board(mem.board)
            turn_game_outcome = game_outcome if mem.board.current_player == spg.board.current_player else -game_outcome

            for board, visit_counts in ChessGame.symmetric_variations(encoded_board, mem.visit_counts):
    ...
    def _log_game(self, spg: SelfPlayGame, result: float) -> None:
        moves = ','.join([str(ChessGame.encode_move(move)) for move in spg.played_moves])
