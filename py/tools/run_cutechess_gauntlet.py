...
def parse_arguments() -> GauntletConfiguration:
    parser = argparse.ArgumentParser(description='Run a paired-color Cute Chess calibration gauntlet.')
    ...
    parser.add_argument('--stockfish-hash-mib', type=int, default=16)
...
def validate_configuration(configuration: GauntletConfiguration) -> None:
    ...
    if configuration.stockfish_threads < 1:
        raise ValueError('stockfish_threads must be positive.')
    if configuration.stockfish_hash_mib < 1:
        raise ValueError('stockfish_hash_mib must be positive.')
...
def command_for_opponent(configuration: GauntletConfiguration, stockfish_elo: int) -> list[str]:
    ...
    return [
        ...
        f'option.Threads={configuration.stockfish_threads}',
        f'option.Hash={configuration.stockfish_hash_mib}',
        '-each',
