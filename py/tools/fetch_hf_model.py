...
def parse_arguments() -> DownloadRequest:
    parser = argparse.ArgumentParser(description='Fetch a TorchScript model from Hugging Face.')
...
def validate_request(request: DownloadRequest) -> None:
    if request.revision == '':
        raise ValueError('revision must be omitted or contain a branch, tag, or commit.')
