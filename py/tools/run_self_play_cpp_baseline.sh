...
if [[ "$#" -lt 1 ]]; then
    ...
    echo "Workload overrides: SEARCHES_PER_PLY, PARALLEL_SEARCHES, MAXIMUM_BATCH_SIZE, INFERENCE_TIMEOUT_MICROSECONDS, CACHE_CAPACITY"
...
output_directory="${output_root}/self-play-cpp-baseline-${topology_name}-${workload_name}-timeout${inference_timeout_microseconds}us-affinity${affinity_mode}-${run_timestamp}"
