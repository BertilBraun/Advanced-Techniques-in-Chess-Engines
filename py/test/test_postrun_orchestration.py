...
def test_ladder_commands_pin_the_protocol_and_disjoint_devices(tmp_path: Path) -> None:
    ...
    assert commands[0].command[commands[0].command.index('--inference-batch-size') + 1] == '2'
    assert commands[1].command[commands[1].command.index('--inference-batch-size') + 1] == '3'
