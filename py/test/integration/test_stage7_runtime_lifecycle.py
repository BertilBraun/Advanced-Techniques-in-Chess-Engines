...
from src.az.runtime.orchestrator import (
    ReplayPublisher,
    RuntimeOrchestrator,
    WorkerEntrypoint,
)
...
def _clean_worker(
    ...
    _send(
        queue,
        WorkerReady(
            kind="worker_ready", worker_index=0, process_id=os.getpid(), model_version=0
        ),
    )
    _send(
        queue,
...
def _gate_specifications(
    ...
    evidence_paths = tuple(
        (temporary_directory / f"gate-evidence-{worker_index}.json").resolve()
...
def test_nontimeout_completion_records_clean_lifecycle(tmp_path: Path) -> None:
    ...
    assert not any(
        child.name.startswith("az-self-play-")
        for child in multiprocessing.active_children()
    )


@pytest.mark.integration
def test_experiment_clock_starts_only_after_slow_worker_is_ready(
    tmp_path: Path,
) -> None:
...
@pytest.mark.integration
def test_workers_observe_one_shared_epoch_only_after_every_worker_is_ready(
    tmp_path: Path,
) -> None:
    specifications, evidence_paths = _gate_specifications(
        tmp_path, fail_first_worker=False
    )
    result = _orchestrator(
        tmp_path,
        _gated_worker,
    ...
    observations = tuple(
        GateEvidence.model_validate_json(path.read_text(encoding="utf-8"))
    ...
    assert (
        len({observation.experiment_start_monotonic_ns for observation in observations})
        == 1
    )
...
@pytest.mark.integration
def test_startup_failure_releases_gate_without_beginning_experiment(
    tmp_path: Path,
) -> None:
    specifications, evidence_paths = _gate_specifications(
        tmp_path, fail_first_worker=True
    )
    ...
    observation = GateEvidence.model_validate_json(
...
def test_resumed_workers_observe_cumulative_schedule_epoch(tmp_path: Path) -> None:
    ...
    evidence = tuple(
        GateEvidence.model_validate_json(path.read_bytes()) for path in evidence_paths
    )
    observed_elapsed = tuple(
        (time.monotonic_ns() - item.experiment_start_monotonic_ns) / 1_000_000_000
