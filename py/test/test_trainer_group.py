...
def test_qat_checkpoint_save_refreshes_live_sidecar_identity(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    ...
    runtime = SimpleNamespace(
        ...
        game=SimpleNamespace(
            self_play_configuration=SimpleNamespace(inference=SimpleNamespace(inference_batch_size=1, backend=object()))
        ),
    )
    ...
    monkeypatch.setattr(
        ...
        lambda *arguments, quantization_configuration, **keyword_arguments: checkpoint,
    )
