...
class FakeGame:
    def __init__(self, moves_uci: tuple[str, ...], result_move: str = "e2e4") -> None:
    ...
    def analyze(self, request: SearchRequest) -> FakeResult:
        ...
        return FakeResult(self.result_move)
...
class FakeEngine:
    ...
    def new_game(self, starting_fen: str, moves_uci: tuple[str, ...]) -> FakeGame:
        game = (
            StoppableFakeGame(moves_uci, self.result_move)
            if self.stoppable
            else FakeGame(moves_uci, self.result_move)
        )
...
@pytest.mark.parametrize(
    "command", ["position startpos moves e2e5", "position fen broken"]
)
...
@pytest.mark.parametrize(
    ("milliseconds", "seconds"), [(1_000, 1), (10_999, 10), (30_000, 30)]
)
def test_parse_move_time_accepts_contract_range(
    milliseconds: int, seconds: int
) -> None:
    assert parse_move_time_seconds(f"go movetime {milliseconds}") == seconds


@pytest.mark.parametrize(
    "command", ["go movetime 999", "go movetime 30001", "go wtime 1000"]
)
...
def test_parse_search_mode() -> None:
    assert (
        parse_search_mode("setoption name SearchMode value policy") is SearchMode.POLICY
    )
