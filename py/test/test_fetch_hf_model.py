...
def test_validate_request_rejects_non_torchscript_or_unsafe_model(
    ...
    with pytest.raises(ValueError):
        validate_request(DownloadRequest('owner/repository', None, filename, Path('models'), None))
...
def test_write_provenance_records_resolved_revision(tmp_path: Path) -> None:
    ...
    assert f'resolved_revision={"a" * 40}' in provenance_path.read_text(encoding='utf-8')
