...
CONFIGURATION_DIRECTORY = Path("configs/go")
AUTHORING_PATHS = tuple(sorted(CONFIGURATION_DIRECTORY.glob("*.authoring.json")))


def fixed_authoring() -> AuthoringGoExperimentConfiguration:
    return load_authoring_configuration(
        CONFIGURATION_DIRECTORY / "go-7x7-fixed.authoring.json"
    )
...
def rebuild_configuration(
    ...
    return validate_resolved_configuration(
        configuration.model_copy(update={"replay": replay, "training": training})
    )
...
def test_representative_authoring_and_resolved_configurations_match(path: Path) -> None:
    ...
    assert resolved.game_configuration.board_size == 7
    assert resolved.game_configuration.action_count == 50
    assert resolved.game_configuration.input_plane_count == 17


def test_resolution_materializes_every_root_category() -> None:
    authoring = fixed_authoring()
    assert set(authoring.experiment.model_fields_set) < set(
        type(authoring.experiment).model_fields
    )

    serialized = resolve_configuration(authoring).model_dump(
        mode="json", exclude_unset=False
    )

    assert set(serialized) == set(GoExperimentConfiguration.model_fields)
    assert serialized["game"] == "go"
    assert serialized["game_configuration"]["scoring_rule"] == "area"
    assert serialized["game_configuration"]["ko_rule"] == "positional_superko"
    assert serialized["game_configuration"]["suicide_rule"] == "illegal"
    assert serialized["game_configuration"]["pass_exempt_from_superko"] is True
    assert (
        serialized["game_configuration"]["score_comparison"] == "doubled_integer_points"
    )
...
def test_go_configuration_rejects_unknown_payload_schema(
    payload_schema_version: int,
) -> None:
    configuration = resolve_configuration(fixed_authoring())
    replay = configuration.replay.model_copy(
        update={"payload_schema_version": payload_schema_version}
    )
...
def test_go_objective_l2_and_optimizer_decay_are_separate_experimental_choices() -> (
    None
):
    configuration = resolve_configuration(fixed_authoring())
    objective = configuration.training.objective.model_copy(
        update={"l2_regularization_weight": 0.001}
    )
    training = configuration.training.model_copy(update={"objective": objective})

    with pytest.raises(
        ValidationError, match="mutually exclusive experimental choices"
    ):
...
def test_native_simulation_count_boundary_is_explicit() -> None:
    assert (
        FixedSearchBudget(
            kind="fixed", simulations=NATIVE_SIMULATION_COUNT_MAXIMUM
        ).simulations
        == NATIVE_SIMULATION_COUNT_MAXIMUM
    )
    assert (
...
def test_serialized_configuration_rejects_non_finite_nested_floats(
    ...
    with pytest.raises(ValidationError, match="finite number"):
        validate_authoring_configuration(candidate)
...
def test_resolved_configuration_is_frozen() -> None:
    ...
    with pytest.raises(ValidationError, match="frozen"):
        configuration.game_configuration.komi_half_points = 13
...
def test_forbids_unknown_fields_at_nested_boundaries() -> None:
    ...
    with pytest.raises(ValidationError, match="legacy_playout_cap"):
        validate_authoring_configuration_json(json.dumps(candidate))
...
def test_go_rule_invariants_are_fixed(
    ...
    candidate["game_configuration"][field_name] = value

    with pytest.raises(ValidationError, match=message):
        validate_resolved_configuration_json(json.dumps(candidate))


@pytest.mark.parametrize("board_size", [3, 5, 8, 13])
def test_go_configuration_accepts_general_board_sizes(board_size: int) -> None:
    candidate = resolve_configuration(fixed_authoring()).model_dump(mode="json")
    candidate["game_configuration"]["board_size"] = board_size
    candidate["game_configuration"]["safety_ply_cap"] = board_size * board_size * 3

    resolved = validate_resolved_configuration_json(json.dumps(candidate))

    assert resolved.game_configuration.board_size == board_size
    assert resolved.game_configuration.action_count == board_size * board_size + 1


def test_go_configuration_rejects_unrepresentable_action_space() -> None:
    candidate = resolve_configuration(fixed_authoring()).model_dump(mode="json")
    candidate["game_configuration"]["board_size"] = 46_341
    candidate["game_configuration"]["safety_ply_cap"] = 2**31 - 1
...
def test_go_configuration_accepts_largest_representable_action_space() -> None:
    candidate = resolve_configuration(fixed_authoring()).model_dump(mode="json")
    candidate["game_configuration"]["board_size"] = 46_340
    candidate["game_configuration"]["safety_ply_cap"] = 46_340**2

    resolved = validate_resolved_configuration_json(json.dumps(candidate))

    assert resolved.game_configuration.action_count == 46_340**2 + 1


def test_komi_is_serialized_as_integer_half_points() -> None:
    candidate = fixed_authoring().model_dump(mode="json")
    candidate["game_configuration"]["komi_half_points"] = 7.5
...
def test_go_native_komi_boundaries_are_accepted(komi_half_points: int) -> None:
    candidate = fixed_authoring().model_dump(mode="json")
    candidate["game_configuration"]["komi_half_points"] = komi_half_points

    configuration = validate_authoring_configuration_json(json.dumps(candidate))

    assert configuration.game_configuration.komi_half_points == komi_half_points


def test_go_native_positive_maxima_are_accepted() -> None:
    candidate = fixed_authoring().model_dump(mode="json")
    candidate["game_configuration"]["safety_ply_cap"] = 2**31 - 1
    candidate["game_configuration"]["history_length"] = 1_024

    configuration = validate_authoring_configuration_json(json.dumps(candidate))

    assert configuration.game_configuration.safety_ply_cap == 2**31 - 1
    assert configuration.game_configuration.history_length == 1_024
...
def test_progressive_stages_require_zero_and_strict_order() -> None:
    candidate = load_authoring_configuration(
        CONFIGURATION_DIRECTORY / "go-7x7-progressive.authoring.json"
...
def test_mixed_search_requires_distinct_caps() -> None:
    candidate = load_authoring_configuration(
        CONFIGURATION_DIRECTORY / "go-7x7-mixed.authoring.json"
    ).model_dump(mode="json")
...
def test_canonical_hash_is_stable_across_round_trip_and_key_order() -> None:
    ...
    round_tripped = validate_resolved_configuration_json(reordered)
...
def test_progressive_model_schedule_uses_discriminator_and_ordering() -> None:
    ...
    progressive = validate_resolved_configuration_json(json.dumps(candidate))
...
def test_every_authoring_category_override_flows_to_resolved_configuration() -> None:
    ...
    candidate["game_configuration"]["komi_half_points"] = 13
    ...
    assert resolved.game_configuration.komi_half_points == 13
...
def _initialize_git_repository(path: Path) -> None:
    ...
    subprocess.run(
        ("git", "add", "tracked.txt", "requirements.lock"), cwd=path, check=True
    )
...
def test_manifest_rejects_dirty_source_when_policy_requires_clean(
    tmp_path: Path,
) -> None:
    _initialize_git_repository(tmp_path)
    (tmp_path / "tracked.txt").write_text("dirty\n", encoding="utf-8")
...
def test_manifest_records_dirty_patch_when_policy_allows_it(tmp_path: Path) -> None:
    ...
    configuration = validate_resolved_configuration_json(json.dumps(candidate))
...
def test_manifest_requires_existing_matching_dependency_lock(tmp_path: Path) -> None:
    ...
    mismatched = make_test_dependency_declaration(tmp_path).model_copy(
        update={"lock_file_sha256": "a" * 64}
    )
...
def test_dependency_records_follow_manifest_policy(tmp_path: Path) -> None:
    ...
    no_versions_configuration = validate_resolved_configuration_json(
...
def test_dependency_lock_parser_records_complete_sorted_inventory(
    tmp_path: Path,
) -> None:
    lock_file = tmp_path / "requirements.lock"
...
def test_manifest_records_configuration_environment_and_seed_contract(
    tmp_path: Path,
) -> None:
    _initialize_git_repository(tmp_path)
    configuration = resolve_configuration(fixed_authoring())
    ...
    assert type(manifest.model_validate_json(manifest.model_dump_json())) is type(
        manifest
    )
...
def test_cli_validates_resolves_and_prints_without_starting_run(
    ...
    assert validate_resolved_configuration_json(printed) == load_resolved_configuration(
        resolved_path
    )
...
def test_cli_prints_manifest_without_starting_run(
    ...
    assert exit_code == 0
    assert printed["configuration_sha256"] == model_sha256(
        resolve_configuration(fixed_authoring())
    )
