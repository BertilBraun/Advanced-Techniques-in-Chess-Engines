...
def test_training_model_keeps_auxiliary_heads_but_jit_inference_model_trims_them(
    ...
    inference_policy, inference_wdl, inference_search_budget = inference_model(torch.zeros((2, 3, 3, 3)))
    ...
    assert tuple(output.shape for output in loaded_output.auxiliary_logits) == ((2, 10), (2, 1), (2, 1))
    assert inference_policy.shape == (2, 10)
    assert inference_wdl.shape == (2, 3)
    assert inference_search_budget.shape == (2, 1)
...
def test_spatial_policy_checkpoint_and_trimmed_jit_preserve_inference_abi(tmp_path: Path) -> None:
    ...
    inference_policy, inference_wdl, inference_search_budget = inference_model(inputs)
    ...
    torch.testing.assert_close(inference_search_budget, torch.zeros((2, 1)))
