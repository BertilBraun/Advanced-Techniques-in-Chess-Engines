...
def create_optimizer(model: Network, type: OptimizerType) -> torch.optim.Optimizer:
    if type == 'sgd':
        return torch.optim.SGD(model.parameters(), lr=0.01, momentum=0.9, weight_decay=0.0001, nesterov=True)
    elif type == 'adamw':
        return torch.optim.AdamW(model.parameters(), lr=0.001, weight_decay=0.0001, amsgrad=True)
