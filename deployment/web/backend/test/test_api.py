...
class FakeGame:
    ...
    def analyze(self, request: AnalysisRequest) -> AnalysisResult:
        ...
        return AnalysisResult(
            ...
            value=0.25,
            outcome=OutcomePrediction(win=0.5, draw=0.3, loss=0.2),
            candidates=(
                CandidateAnalysis(
                    move_uci=move,
                    policy_prior=0.4,
...
def test_turn_uses_complete_history_and_returns_authoritative_moves() -> None:
    ...
    created = client.post('/api/games', json={'starting_fen': chess.STARTING_FEN, 'moves_uci': []})
    ...
    response = client.post(
        ...
        json={
            ...
            'moves_uci': [],
            'human_move_uci': 'e2e4',
            'analysis': {'type': 'timed_mcts', 'seconds': 1},
        },
    )
    ...
    assert payload['analysis']['searches'] == 8
    assert payload['analysis']['outcome'] == {'win': 0.5, 'draw': 0.3, 'loss': 0.2}
...
def test_illegal_move_and_invalid_time_are_rejected() -> None:
    client = TestClient(create_app(GameService(FakeEngine()), ['https://chess.example']))
    token = client.post('/api/games', json={'starting_fen': chess.STARTING_FEN, 'moves_uci': []}).json()['game_token']
    request = {
        'starting_fen': chess.STARTING_FEN,
        'moves_uci': [],
        'human_move_uci': 'e2e5',
        'analysis': {'type': 'timed_mcts', 'seconds': 31},
    }
    assert client.post(f'/api/games/{token}/turns', json=request).status_code == 422
    request['analysis']['seconds'] = 1
    assert client.post(f'/api/games/{token}/turns', json=request).status_code == 422


def test_analysis_variants_reject_irrelevant_fields() -> None:
    client = TestClient(create_app(GameService(FakeEngine()), ['https://chess.example']))
    token = client.post('/api/games', json={'starting_fen': chess.STARTING_FEN, 'moves_uci': []}).json()['game_token']
    request = {
        'starting_fen': chess.STARTING_FEN,
        'moves_uci': [],
        'human_move_uci': None,
        'analysis': {'type': 'policy', 'seconds': 1},
    }
