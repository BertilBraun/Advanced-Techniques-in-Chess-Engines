...
import chess
from play.mcts.mcts import ChessState
...
def evaluate_position_static(state: ChessState, moves: List[chess.Move]) -> List[float]:
    global total_evaluations
    total_evaluations += len(moves)

    return [
        _evaluate_move(state.turn, state.state, move)
        for move in moves
    ]


def _evaluate_move(turn: chess.Color, board: chess.Board, move: chess.Move) -> float:
