...
static_assert(az::games::GameState<FixtureState>);
static_assert(az::search::MAXIMUM_SIMULATION_COUNT == std::numeric_limits<int32>::max());
...
struct WideActionState {
    ...
    [[nodiscard]] action_type actionCount() const {
        return static_cast<action_type>(az::search::MAXIMUM_SIMULATION_COUNT) + 1;
    }
...
static_assert(az::games::GameState<WideActionState>);
...
public:
    ...
    [[nodiscard]] az::inference::InferenceResult
    evaluate(const az::inference::InferenceRequest<int32> &request) {
        ++calls;
        return {
...
static_assert(az::inference::SynchronousEvaluator<FixtureEvaluator, int32>);

az::search::FixedPuctConfiguration configuration(int32 simulations = 1, double discount = 1.0,
...
void testMaskingBackupAndExactAccounting() {
    ...
    const auto result = az::search::FixedPuctSearch<FixtureState>::run(
    ...
    assert(result.telemetry.budgetClass == az::search::SearchBudgetClass::Fixed);
    assert(result.telemetry.stopReason == az::search::SearchStopReason::FullBudget);
...
void testParentValueFpuUsesExpansionValueBeforeFirstBackup() {
    ...
    searchConfiguration.fpuPolicy = az::search::FpuPolicy::ParentValue;
    const auto result = az::search::FixedPuctSearch<FixtureState>::run(
        FixtureState{}, evaluator, terminalValue, searchConfiguration);
...
void testReducedParentFpuUsesVisitedPolicyMass() {
    ...
    evaluator.leaf_value = -1.0;
    auto searchConfiguration = configuration(1);
    searchConfiguration.fpuPolicy = az::search::FpuPolicy::ReducedParentValue;
...
void testAdaptiveStoppingChecksOnlyConfiguredIntervals() {
    ...
    assert(result.telemetry.stopReason == az::search::SearchStopReason::AdaptiveConfidence);
    assert(result.telemetry.budgetClass == az::search::SearchBudgetClass::Fixed);
}
...
void testBudgetMetadataDoesNotChangeSearch() {
    ...
    searchConfiguration.budgetClass = az::search::SearchBudgetClass::MixedFast;
    searchConfiguration.policyTargetWeight = 0.0;
    const auto result = az::search::FixedPuctSearch<FixtureState>::run(
        FixtureState{}, evaluator, terminalValue, searchConfiguration);
    assert(result.telemetry.actualSimulations == 3);
    assert(result.telemetry.budgetClass == az::search::SearchBudgetClass::MixedFast);
    assert(!result.telemetry.policyTargetEligible);
...
void testAdaptiveCadenceAtCapReportsFullBudget() {
    ...
    const auto prefix = az::search::FixedPuctSearch<FixtureState>::run(
...
void testTerminalRootIsTypedAndDoesNotInfer() {
    ...
    assert(result.telemetry.stopReason == az::search::SearchStopReason::TerminalRoot);
    assert(!result.telemetry.policyTargetEligible);
    assert(result.telemetry.policyTargetWeight == 0.0);
}
...
void testTemperatureAndRepeatability() {
    ...
    const auto first = az::search::FixedPuctSearch<FixtureState>::run(
        FixtureState{}, firstEvaluator, terminalValue, configuration(32, 1.0, 1e-300, 81));
    const auto second = az::search::FixedPuctSearch<FixtureState>::run(
    ...
    const auto deterministic = az::search::FixedPuctSearch<FixtureState>::run(
...
template <typename Mutation> void expectInvalidInference(Mutation mutation) {
    ...
    bool rejected = false;
    try {
        (void) az::search::FixedPuctSearch<FixtureState>::run(FixtureState{}, evaluator,
...
void testRootNoiseIsSeeded() {
    ...
    const auto different = az::search::FixedPuctSearch<FixtureState>::run(
...
void testRandomPurposesUseIndependentStreams() {
    ...
    const auto baseline = az::search::FixedPuctSearch<FixtureState>::run(
    ...
    for (uint64 actionSeed = 2; actionSeed < 64; ++actionSeed) {
        ...
        FixtureEvaluator evaluator;
        const auto changed = az::search::FixedPuctSearch<FixtureState>::run(
...
void testInvalidConfigurationAndBatchContract() {
    ...
    az::search::SeededRandom random(1);
    ...
    try {
        (void) az::search::FixedPuctSearch<WideActionState>::run(
...
void testBatchValidationAndOrderedAssociation() {
    using az::inference::InferenceBatch;
    using az::inference::InferenceBatchResult;
    ...
    az::inference::validateBatchResult(batch, validResult);

    expectInvalidBatch([] { az::inference::validateBatch(InferenceBatch<int32>{.requests = {}}); });
    expectInvalidBatch([] {
        az::inference::validateBatch(
...
void testPrefixTraceCapturesExactCheckpointsWithoutChangingSearch() {
    ...
    const auto traced = az::search::FixedPuctSearch<FixtureState>::run(
