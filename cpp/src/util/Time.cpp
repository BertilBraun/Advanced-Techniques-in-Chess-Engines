...
TimeItGuard::~TimeItGuard() {
    ...
    if (!callStack.empty()) {
        std::string parent = callStack.back();
...
void resetTimes(TensorBoardLogger *logger, int iteration) {
    ...
    float totalTime = 0.0;
    for (const auto &[key, value] : functionTimes) {
        totalTime += value;
    }
    ...
    if (totalTime > 0) {
        ...
        for (const auto &[key, totalFuncTime] : sorted) {
            float percent = totalFuncTime / globalTotalTime * 100.0;
            if (logger) {
                logger->addScalar(std::string("function_percent_of_execution_time/") + key,
                                  iteration, percent);
                logger->addScalar(std::string("function_total_time/") + key, iteration,
                                  totalFuncTime);
                logger->addScalar(std::string("function_total_invocations/") + key, iteration,
                                  static_cast<float>(globalFunctionInvocations[key]));
            }

            // Calculate local percentage for the function, if it exists in function_times.
            float localPercent =
                functionTimes.count(key) ? functionTimes[key] / totalTime * 100.0 : 0.0;
            log(localPercent, "% (total", percent, "% on", globalFunctionInvocations[key],
                "invocations)", key);
        }

        float totalElapsed = std::chrono::duration<float>(
                                 std::chrono::high_resolution_clock::now() - startTimingTime)
                                 .count();
        if (logger) {
            logger->addScalar("function_time_total_traced_percent", iteration,
                              globalTotalTime / totalElapsed * 100.0);
        }
