...
BoundSelfPlayClasses<Game> bindSelfPlay(py::module_ &module, const SelfPlayBindingNames names) {
    ...
    search
        ...
        .def("new_root", [](Search &selectedSearch, const Position &position) {
            return selectedSearch.newRoot(position);
        }, py::arg("position"))
        .def("request", [](const Search &, Root selectedRoot, const bool fullSearch) {
            return Request(std::move(selectedRoot), fullSearch);
        }, py::arg("root"), py::arg("full_search"))
        .def("search", &Search::search, py::arg("requests"),
             py::arg("collect_statistics") = false, py::call_guard<py::gil_scoped_release>())
