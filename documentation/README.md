...
- [Onboarding](ONBOARDING.md) — read-in-this-order path for a new contributor or agent, plus setup, node
  provisioning and the validation commands.
- [Experiments](experiments/README.md) — the narrative ledger of what was tried and what survived; summaries
  only, with the measured artifacts under `benchmarks/`.
...
| Cross-run verdict worth remembering | `experiments/README.md`, pointing at the measurement |
| Figure used by the repository README | `showcase/`, generated by `py/tools/render_showcase_plots.py` |
...
- [`evidence/`](evidence/README.md) — dated per-run and per-node records, including the four-day freeze.
- [`showcase/`](showcase/README.md) — the generated figures the repository README embeds, with their source
  archives and the one-command regeneration path.
