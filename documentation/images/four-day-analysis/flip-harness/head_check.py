...
from src.training.network import Network, NetworkParams, AttentionNetworkParams, Chess76PlaneDirectPolicyHeadConfiguration
...
conv = Network(NetworkParams(policy_head=Chess76PlaneDirectPolicyHeadConfiguration(), num_layers=2, hidden_size=16), dev, dims)
with torch.no_grad():
    for m in conv.modules():
        if isinstance(m, torch.nn.Conv2d) and m.kernel_size == (3, 3):
            w = m.weight.clone(); w[:, :, :, :] = 0; w[:, :, 1, 1] = m.weight[:, :, 1, 1]; m.weight.copy_(w)
...
att = Network(AttentionNetworkParams(policy_head=Chess76PlaneDirectPolicyHeadConfiguration(), num_layers=2, embedding_size=16, num_heads=2, feedforward_size=32), dev, dims)
with torch.no_grad():
...
print('example: black move e8g8 (castle) ->', r['moves']['e8g8'][0], 'plane', r['moves']['e8g8'][0] // 64, 'from sq', r['moves']['e8g8'][0] % 64, '(e1=4 canonical)')
