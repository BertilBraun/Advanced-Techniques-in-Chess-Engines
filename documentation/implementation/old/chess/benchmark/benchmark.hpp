...
void generateMovesBenchmark(int iterations, int depth) {
  ...
  std::cout << "Average time for generating moves and pushing/popping a move: "
            << averageTime << " seconds.\n";
...
void copyStateBenchmark(int iterations) {
  ...
  double averageTime = totalDuration.count() / iterations;
  std::cout << "Average time for copying board state: " << averageTime
            << " seconds.\n";
  std::cout << "Total time for copying board state: " << totalDuration.count()
            << " seconds.\n";
}
